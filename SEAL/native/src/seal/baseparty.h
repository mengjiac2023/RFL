...
public:
    ...
    virtual void gather_and_public_pk(vector<int64_t> lg, vector<int> id, int64_t* mod)=0;
    virtual void partial_decrypt(Ciphertext ctx)=0;
    virtual void gather_and_public_dec(vector<int64_t> lagr, vector<int> indices, int64_t* lagr_lcm)=0;
    virtual void mask_share(int other_id, int64_t weight, int64_t lcm_, Poly& res)=0;
    virtual void update_outbox(vector<int64_t> destination_parties)=0;
    virtual void outbox_push(ZeroSharePRNG* z_ij)=0;
    virtual MetaPoly& gen()=0;
};