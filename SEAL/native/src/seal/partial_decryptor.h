...
public:
    ...
    void dot_product_ct_sk_array(Ciphertext &encrypted, Poly& destination, MemoryPoolHandle pool)
    {
        ...
        if (encrypted_size == 2)
        {
            ...
            {
                set_uint(c1, coeff_count, destination.data());
                ntt_negacyclic_harvey(destination.data(), ntt_tables[0]);
                ntt_negacyclic_harvey(c0, ntt_tables[0]);
                destination.multiply_polynomial2(sk, destination);
                SEAL_ITERATE(iter(destination.data(), c0),coeff_count, [&](auto I){
                    // get<0>(I) = add_uint_mod(get<0>(I), get<1>(I), coeff_modulus[0].value());
                    get<0>(I) += get<1>(I);
                    get<0>(I) %= coeff_modulus[0].value();
                });
                inverse_ntt_negacyclic_harvey(c0, ntt_tables[0]);
            }
        }
    }
    void bfv_decrypt( Ciphertext &encrypted, Poly &tmp_dest_modq)
    {
        ...
        inverse_ntt_negacyclic_harvey(tmp_dest_modq.data(), ntt_tables[0]);
